var character_attack = {
    ...
    getAttack: function(spell_name){
        for(var i = 0; i < character_attack.spell_list.length; i++){
    ...
    attack: function(monster_x,monster_y,attack_name){
        var damage = 0;
        var attack = character_attack.getAttack(attack_name);
        game.character.current_mana = game.character.current_mana - attack.mana_cost;
        for(var i = 0; i < game.character.spells.length; i++){
        ...
        world.get(monster_x,monster_y).npc.current_health -= damage;
        if(world.get(monster_x,monster_y).npc.current_health<=0){
            var loot = world.get(monster_x,monster_y).npc.loot;
            var quest_item = world.get(monster_x,monster_y).npc.quest_item;
            world.get(monster_x,monster_y).npc = null;
