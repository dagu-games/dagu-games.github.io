  ...
  var diceUtils = Object.freeze({
    ...
    get equateNumbers() {
      ...
      return function (a, b) {
        ...
        a = _this2.isNumeric(a) ? parseFloat(a) : 0;
        b = _this2.isNumeric(b) ? parseFloat(b) : 0; // only carry out operation if we have both values
     ...
     * @returns {*}
     */
    trimOperator: function trimOperator(formula) {
  ...
  */
  ...
  function factory$q(type, config, load, typed, math) {
    ...
    FunctionNode.prototype.toString = function (options) {
      ...
      if (options && _typeof$9(options.handler) === 'object' && hasOwnProperty$5(options.handler, name)) {
    ...
    FunctionNode.prototype.toTex = function (options) {
      ...
      if (options && _typeof$9(options.handler) === 'object' && hasOwnProperty$5(options.handler, this.name)) {
        // callback is a map of callback functions
  ...
  var DiceRoll = function () {
    ...
    var _parsedDice = Symbol('parsedDice');

    var _resetTotals = Symbol('resetTotals');
    ...
    var _successes = Symbol('successes');
    /**
     * The roll total
     *
     * @type {symbol}
    ...
    var diceRollMethods = {
       ...
       * @returns {*}
       */
      "default": function _default(sides) {
       ...
       * @returns {number}
       */
      fudge: function fudge(numNonBlanks) {
        var total = 0;
    ...
    var isComparePoint = function isComparePoint(comparePoint, value) {
    ...
    var getSuccessStateValue = function getSuccessStateValue(value, comparePoint) {
      return isComparePoint(comparePoint, value) ? 1 : 0;
    };

    var parseDie = function parseDie(notation) {
      var parsed = []; // parse the notation and find each valid dice (and any attributes)

      var pattern = DiceRoll.notationPatterns.get('notation', 'g');
      var match;
      ...
      while ((match = pattern.exec(notation)) !== null) {
        var die = {
          ...
          sides: diceUtils.isNumeric(match[3]) ? parseInt(match[3], 10) : match[3],
    ...
    var rollDie = function rollDie(die) {
      var dieRolls = []; // list of roll results for the die
      ...
      if (sides) {
        ...
        for (var i = 0; i < die.qty; i++) {
          ...
          do {
            ...
            roll = callback.call(die, sides); // add the roll to our list
    ...
    function () {
      ...
      function DiceRoll(notation) {
        ...
        if (notation instanceof Object) {
          ...
          // store the notation
          this[_notation] = notation.replace(/\s+/g, ''); // empty the current rolls
      ...
      _createClass(DiceRoll, [{
        ...
        value: function value() {
          ...
          var rollIndex = 0,
              prevRolls; // reset the totals and successes
          ...
          var buildFormulas = function buildFormulas(parsedDiceI) {
            var formulas = {
              total: '',
              successes: ''
            }; // loop through each roll and calculate the totals

            parsedDiceI.forEach(function (item) {
              if (Array.isArray(item)) {
                // this is a parenthesis group - loop recursively
                var segmentFormulas = buildFormulas(item),
                    successFormula = diceUtils.trimOperator(segmentFormulas.successes);
                formulas.total += segmentFormulas.total ? "(".concat(segmentFormulas.total, ")") : '';
                formulas.successes += successFormula ? "(".concat(successFormula, ")") : '';
              } else if (_typeof(item) === 'object') {
                // item is a die
                var rolls = _this.rolls[rollIndex] || [],
                    rollValues = [],
                    dieTotal;
                ...
                if (item.pool) {
                  ...
                  rollValues = rolls.map(function (value) {
                ...
                dieTotal = diceUtils.sumArray(rollValues); // if this is a pool dice, add it's successes to the success count
                ...
                formulas.total += dieTotal; // increment the roll index and store the previous rolls / parsed die
                ...
                var prevRollsValues = [],
                    value = item,
                    isPool = false,
                    isPoolModifier = false; // determine the previous roll details for handling things like H/L modifiers
                ...
                if (item === 'H') {
                  ...
                  value = Math.max.apply(Math, _toConsumableArray(prevRollsValues)); // flag that this value needs to be modified to a success/failure value

                  isPoolModifier = true;
                } else if (value === 'L') {
                  // 'L' is equivalent to the lowest roll
                  value = Math.min.apply(Math, _toConsumableArray(prevRollsValues)); // flag that this value needs to be modified to a success/failure value
          ...
          var formulas = buildFormulas(this[_parsedDice]); // if a total formula has been produced, evaluate it and round it to max 2 decimal places
        ...
        value: function roll() {
          ...
          var rollGroups = function rollGroups(group) {
          ...
          this[_rolls] = rollGroups(this[_parsedDice]); // return the rolls;
        ...
        get: function get() {
          ...
          if (this[_parsedDice] && Array.isArray(this.rolls) && this.rolls.length) {
            ...
            var outputRecursive = function outputRecursive(parsedDiceI) {
              return parsedDiceI.map(function (item) {
                if (Array.isArray(item)) {
                  // this is a parenthesis group - loop recursively
                  return "(".concat(outputRecursive(item), ")");
                } else if (_typeof(item) === 'object') {
                  var rolls = _this2.rolls[rollIndex] || [],
                      hasComparePoint = item.comparePoint;
                  var rollOutput = ''; // current roll total - used for totalling compounding rolls

                  var currentRoll = 0;
                  rollOutput += '['; // output the rolls
                  ...
                  rolls.forEach(function (roll, rIndex, array) {
                    ...
                    var delimit = rIndex !== array.length - 1;
                  ...
                  rollOutput += ']'; // increment the roll index
            ...
            output += "".concat(outputRecursive(this[_parsedDice]), " = ").concat(this.total);
          } else {
        ...
        key: "parseNotation",
        value: function parseNotation(notation) {
          var parsed = []; // only continue if a notation was passed

          if (notation) {
            // build the regex for matching a die (including exploding and pool dice etc. notation)
            var dieRegex = DiceRoll.notationPatterns.get('dieFull', 'g'); // split the notation into its parenthesis groupings

            var groups = XRegExp.matchRecursive(notation, '\\(', '\\)', 'g', {
              valueNames: ['between', 'left', 'match', 'right']
            ...
            groups.forEach(function (group) {
              if (group.name === 'match') {
                ...
                var segments = group.value.split(/([+\-\/*])/).filter(Boolean); // loop through each segment and determine what it is
                ...
                segments.map(function (segment) {
                  ...
                  if (XRegExp.test(segment, dieRegex)) {
                    ...
                    parsed.push.apply(parsed, _toConsumableArray(parseDie(segment)));
                  } else {
    ...
    DiceRoll.notationPatterns = function () {
      var strings = {
        /**
        ...
        get numberComparison() {
          return "(".concat(this.comparisonOperators, ")(").concat(this.numberDecimal, ")");
        },
         ...
         */
        get die() {
          return "([1-9]\\d*)?d([1-9]\\d*|%|".concat(this.fudge, ")");
        },
         ...
         */
        get dieFull() {
          return "".concat(this.die).concat(this.explode, "?(?:").concat(this.numberComparison, ")?");
        },
         ...
         */
        get operation() {
          return "(".concat(this.arithmeticOperator, ")(").concat(this.numberDecimal, "(?!\\d*d)|H|L)");
        },
         ...
         */
        get notation() {
          return "(".concat(this.arithmeticOperator, ")?").concat(this.dieFull, "((?:").concat(this.operation, ")*)");
        },
      ...
      return {
         ...
         * @returns {RegExp}
         */
        get: function get(name, flags) {
          var matchWhole = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
          ...
          if (!name) {
            ...
            throw new Error("DiceRoller: Notation pattern name not found: ".concat(name));
  ...
  var DiceRoller = function () {
    ...
    function () {
      ...
      _createClass(DiceRoller, [{
        ...
        key: "rollMany",
        value: function rollMany(notations) {
          var _this2 = this;
        ...
        value: function _import(data) {
          ...
          if (!data) {
            ...
            if (data.log && Array.isArray(data.log)) {
              // loop through each log entry and import it
              data.log.forEach(function (roll) {
                _this3[_log].push(DiceRoll["import"](roll));
              });
            } else if (data.log) {
              throw new Error('DiceRoller: Roll log must be an Array');
            }
